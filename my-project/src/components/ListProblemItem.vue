<template>
    ...
    <!-- Hiển thị nút testcase -->
      <div class="col-span-2">
        <button @click="showAddTestCaseModal" class=" px-4 py-2 rounded bg-white">Thêm Test Case</button>
      </div>

      <!-- Modal thêm test case -->
      <div v-if="isAddTestCaseModalVisible" class="fixed inset-0 overflow-y-auto flex items-center justify-center">
    ...
    <div v-if = "isMessengerTestCaseVisible" class="fixed inset-0 overflow-y-auto flex items-center justify-center">
          <div class="relative p-4 w-1/4 max-w-3xl max-h-full justify-center">
              <!-- Modal content -->
              <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  <!-- Modal header -->
                  <div class="flex items-center p-4 md:p-5 border-b rounded-t dark:border-gray-600 justify-center">
...
const saveTestCase = async (testCase) => {
  try {
    ...
    testCaseMessenger = response.data;
